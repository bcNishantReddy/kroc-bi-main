        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          >
            ...
            <Card className="p-6 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                  <h3 className="font-medium text-muted-foreground">Total Missing Values</h3>
                  <p className="text-foreground">
                    {bundle.data_quality_stats?.missing_values 
                      ? Object.values(bundle.data_quality_stats.missing_values).reduce((a, b) => a + b, 0).toLocaleString()
                      : "N/A"}
                  </p>
                </div>
                <div>
                  <h3 className="font-medium text-muted-foreground">Duplicate Rows</h3>
                  <p className="text-foreground">
                    {bundle.data_quality_stats?.duplicates?.toLocaleString() || "N/A"}
                  </p>
                </div>
                <div>
                  <h3 className="font-medium text-muted-foreground">Total Outliers</h3>
                  <p className="text-foreground">
                    {bundle.data_quality_stats?.outliers 
                      ? Object.values(bundle.data_quality_stats.outliers).reduce((a, b) => a + b, 0).toLocaleString()
                      : "N/A"}
                  </p>
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {stats && Object.entries(stats).map(([stat, value]) => (
                        <div key={stat} className="bg-accent/30 p-4 rounded-lg">
                          ...
                          <p className="font-medium">{typeof value === 'number' ? value.toLocaleString() : 'N/A'}</p>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {bundle.integration_config ? (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  {bundle.integration_config.export_formats && (
                    <div className="bg-accent/30 p-4 rounded-lg">
